...
One main drawback of using topic weight to select representatives is that sometimes high abundance clusters with little variation may also be selected due to high topic weights.
In our analysis, the top representative clusters from each topic are selected after ranking by lift, which is the ratio of estimated topic weights over its empirical frequency. 
The lift metic gives higher weights to clusters that appear less frequently in other topics. 

Below show top clusters with top lift for each topic. These clusters are largely overlapped with the manually-selected clusters in the original paper.
